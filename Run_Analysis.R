...
lVector = (grepl("activityId",colNames) | grepl("subjectId",colNames) | grepl("-mean\\(\\)",colNames) | grepl("-std\\(\\)",colNames));
...
for (i in 1:length(colNames)) 
{
  ...
  colNames[i] = gsub("([Bb]odyaccjerkmag)","BodyAccJerk_Magnitude",colNames[i])
